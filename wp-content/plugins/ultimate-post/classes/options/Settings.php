				...
				if ( $value['type'] != 'heading' && $value['type'] != 'hidden' ) {
					...
					switch ( $value['type'] ) {
						...
						case 'text':
							echo '<div class="ultp-settings-field ultp-settings-input-field">';
								$val = isset( $option_data[ $key ] ) ? $option_data[ $key ] : ( isset( $value['default'] ) ? $value['default'] : '' );
								echo '<input type="text" name="' . esc_attr( $key ) . '" value="' . esc_attr( $val ) . '"/>';
								echo '<p class="description">' . esc_html( $value['desc'] ) . (isset($value['link']) ? '<a target="blank" href="'.esc_html( $value['link'] ).'">'.esc_html( $value['linkText'] ).'</a>' : '' ). '</p>';
							echo '</div>';
							break;
						...
						case 'multiselect':
							echo '<div class="ultp-settings-field">';
							$saved_val = isset( $option_data[ $key ] ) ? $option_data[ $key ] : ( isset( $value['default'] ) ? $value['default'] : array() );
